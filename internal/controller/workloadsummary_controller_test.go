...
var _ = Describe("WorkloadSummary Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind WorkloadSummary")
			err := k8sClient.Get(ctx, typeNamespacedName, workloadsummary)
			if err != nil && errors.IsNotFound(err) {
				resource := &uxv1alpha1.WorkloadSummary{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance WorkloadSummary")
